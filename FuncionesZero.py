# Por Adolfo García, Rodrigo Guerrero, Nicolás Sepulveda y Pablo Maluenda
...
def recomendacion(anime):
    """
    Esta función retorna 10 recomendaciones de acuerdo al parametro ingresado.
    Parametro: 
        'anime': Nombre del anime para el cual obtener recomendaciones, tipo string.
    
    """
    global df_animes_conten
    global similitud_coseno
    
    index = df_animes_conten[df_animes_conten['name'] == anime].index[0]
    distances = sorted(list(enumerate(similitud_coseno[index])),reverse=True,key = lambda x: x[1])
    print(f'\033[1mPor que viste {anime}\nQuizás te interese ver:\033[0m\n')
    count = 1
    for i in distances[0:11]:
        if df_animes_conten.iloc[i[0]]['name'] != anime:
            print(f"\033[1m{count}:\033[0m {df_animes_conten.iloc[i[0]]['name']}")
            count+=1
...
def graficos_por_genero(df):
    ...
    def report_scores(validar, predicho): # Creamos la funcion report_scores.
        '''
        Toma como argumento y_test e y_hat o y_estimado
        Retorna un print con las metricas MSE y R2 score
        '''
        m2_mse = mean_squared_error(validar, predicho).round(0)
        m2_r2 = r2_score(validar, predicho).round(2)
        print("El error cuadrático promedio es: ", m2_mse)
        print("El R-cuadrado es: ", m2_r2)
